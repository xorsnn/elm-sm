...
class NavView extends ResizableWidthView
  ...
  constructor: (state, settings, parser) ->
    ...
    @viewContainer.addClass('sm-marker-panel')
    html = """
           <div class='sm-header'>
             <h2>elm-sm</h2>
           </div>
           <div class='sm-tool-panel'>
    ...
    @view.on 'click', '.list-item', (event) =>
      ...
      else if $(elem).is('.sm-marker-group-label')
        $(elem).parent().toggleClass('collapsed')

    @toolPanel = @mainView.find('.sm-tool-panel')
  ...
  setFile: (file) ->
    ...
    if not gutter.data('zNavPanelMouse')
    ...
    if newElem
      ...
      return @setVisibility() if newElem is @filePanel
  ...
  getPanelState: (panel) ->
    ...
    groups = panel.find(".sm-marker-group")
    groups.each ->
      group = $(this)
      groupLabel = $(this).find('.sm-marker-group-label').text().trim()
      if $(this).hasClass('collapsed')
        state.collapsedGroups.push groupLabel
      if groupLabel is 'Bookmarks' and editor
      ...
      group.find('li.sm-highlight').each ->
        state.highlights[groupLabel] ||= []
        row = editor.getMarker(this.markerId).getStartBufferPosition().row
        state.highlights[groupLabel].push row
    if @state[file] and @state[file].sort isnt undefined
  ...
  setPanelState: (panel, state) ->
    return unless panel and state
    editor = @getFileEditor(panel.data('file'))
    return unless editor

    prevFilePanel = null
    if @filePanel isnt panel
  ...
  addGroup: (label) ->
    ...
    group = @filePanel.find(".sm-marker-group-label:contains(#{label})").siblings('ul.list-tree')
    ...
    html = """
             <li class='list-nested-item sm-marker-group #{collapsed}'>
               <div class='list-item sm-marker-group-label'>
                 <span>#{label}</span>
               </div>
               <ul class='list-tree'>
               </ul>
  ...
  addPanelItem: (groupLabel, label, data) ->
    ...
    if typeof groupLabel isnt 'string'
      data = groupLabel
    ...
    if (not tooltip) and label.length > 28
      tooltip = label.replace(/'/g, '&#39;')
    html = """
           <li id='sm-item-#{@nextId}' class='list-item' title='#{tooltip or ''}'>
             <span #{labelClass}></span>
             <span class='sm-marker-label'>#{label}</span>
           </li>
           """
  ...
  toggleHighlight: (element, editor) ->
    ...
    if element.decoration
      ...
      $(element).removeClass('sm-highlight')
      return element.decoration = null
    else
      $(element).addClass('sm-highlight')
      marker = editor.getMarker(element.markerId)
      decoration = editor.decorateMarker(marker, {type: 'line-number', class: 'sm-highlight'})
  ...
  toggleBookmark: (lineNum, editor, skipHighlight) ->
    ...
    lineText = lineText.trim() or '___ blank line ___'
  ...
  getItemByOrigRow: (row, group) ->
    elem = $()
    if group
      root = @filePanel.find(".sm-marker-group-label:contains(#{group})").siblings('ul.list-tree')
    else
      root = @filePanel
    root.find('li.list-item').each ->
      if this.origRow is row
        elem = $(this)
        return false
      return true
    return elem
